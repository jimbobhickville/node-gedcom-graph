...
function Neo4JManager (bindir, db_path) {
    ...
    this.child_process_stdio = ['ignore', 'pipe', 'pipe'];
};
...
Neo4JManager.prototype._manage_process = function (command) {
    ...
    this.emit('stdout', neo_proc.stdout);
    this.emit('stderr', neo_proc.stderr);
...
Neo4JManager.prototype.import = function (import_args) {
    ...
    this.emit('stdout', import_proc.stdout);
    this.emit('stderr', import_proc.stderr);
